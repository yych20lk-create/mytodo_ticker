...
# ========================================================================
# 5. 构建前端 (web/dist) — 保证最新 UI 打进二进制
# ========================================================================
section "构建前端"
if [[ -d "$PROJECT_DIR/web" ]]; then
    if ! command -v npm >/dev/null 2>&1; then
        echo -e "  ${RED}✗ 需要 npm 以构建 Vue 前端${NC}"
        exit 1
    fi
    (
        cd "$PROJECT_DIR/web"
        ...
        echo -e "  ${RED}✗ 前端构建失败（npm run build），中止${NC}"
        exit 1
    }
    if [[ ! -f "$PROJECT_DIR/web/dist/index.html" ]]; then
        echo -e "  ${RED}✗ 前端构建失败：未生成 web/dist/index.html${NC}"
        exit 1
    fi
    echo -e "  ${GREEN}✓ 前端产物: $PROJECT_DIR/web/dist${NC}"
else
